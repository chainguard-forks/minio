...
To deploy Distributed MinIO on Docker Compose, please download [docker-compose.yaml](https://github.com/chainguard-forks/minio/blob/master/docs/orchestration/docker-compose/docker-compose.yaml?raw=true) and [nginx.conf](https://github.com/chainguard-forks/minio/blob/master/docs/orchestration/docker-compose/nginx.conf?raw=true) to your current working directory. Note that Docker Compose pulls the MinIO Docker image, so there is no need to build MinIO from source when using Docker. For non-Docker deployments, MinIO community edition is now source-only and can be installed via `go install github.com/chainguard-forks/minio@latest`. Then run one of the below commands
